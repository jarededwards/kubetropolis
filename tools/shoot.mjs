...
const probe = await send('Runtime.evaluate', {
  expression: `JSON.stringify({
    ...
    now:Number((window.KUBETROPOLIS?.sim?.state?.now ?? -1).toFixed(1)),
    rev:window.KUBETROPOLIS?.sim?.state?.etcd?.revision ?? -1,
    pods:(()=>{const o=window.KUBETROPOLIS?.sim?.state?.etcd?.objects;if(!o)return null;const c={total:0,running:0,ready:0,pending:0};for(const v of o.values()){if(v.kind!=='Pod')continue;c.total++;if(v.status.phase==='Running')c.running++;if(v.status.ready)c.ready++;if(v.status.phase==='Pending')c.pending++}return c})(),
    nodesReady:window.KUBETROPOLIS?.sim?.state?.vitals?.nodesReady ?? -1,
    pullsActive:window.KUBETROPOLIS?.sim?.state?.vitals?.imagePullsActive ?? -1,
    watchLagRev:window.KUBETROPOLIS?.sim?.state?.vitals?.watchMaxLagRev ?? -1,
  })`, returnByValue: true,
})
console.log('=== PROBE ===\n' + probe.result.value)
